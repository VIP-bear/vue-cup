                ...
                border>
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            <el-button @click="publishGoods(scope.$index)" size="large" type="danger" style="margin-left:50px;">发布</el-button>
...
export default {
    ...
    created(){
      this.$axios.get('http://localhost:9528/admin/product/check/0/6')
            .then((resp => (
    ...
    methods: {
      page(currentPage){
        this.$axios.get('http://localhost:9528/admin/product/check/'+(currentPage-1)+'/6')
            .then((resp => (
                this.tableData = resp.data.content,
                this.total = resp.data.totalElements
            )))
      ...
      publishGoods(index) {
        this.$axios.get('http://localhost:9528/admin/publish/'+this.tableData[index].id)
        .then((resp => (
